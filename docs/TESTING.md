...
The scheduled Minecraft update workflow checks Mojang's release manifest but does not create a branch immediately. For a new or not-yet-attested release it first resolves candidate dependencies, builds all three loaders, runs every two-minute in-world gate, and validates the updated compatibility data. This probe job has read-only repository permissions and no persisted Git credentials. It exports a narrowly scoped patch only after every check passes; a separate publish job with write permission verifies and applies that exact patch. A failed dependency lookup, build, gate, or evidence check fails the workflow and uploads the available client logs without changing repository branches.
